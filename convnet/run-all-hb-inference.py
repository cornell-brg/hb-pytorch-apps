...
for i in range(len(route)): # ttl 20
  cmd = copy.deepcopy(route)
  cmd[i]['offload'] = True
  fancy_print(cmd)
  print()
  name = "test_inference_kernel_%03d_hb" % i
  sh_cmd = "mkdir -p " + name
  print(sh_cmd)
  os.system(sh_cmd)
  with open(name + "/inference_kernel.json", 'w') as outfile:
    json.dump(cmd, outfile, indent=4, sort_keys=True)
  sh_cmd = "ln -s ../inference_cosim.py " + name + "/inference_cosim.py"
  print(sh_cmd)
  os.system(sh_cmd)
  sh_cmd = "ln -s ../data " + name + "/data"
  print(sh_cmd)
  os.system(sh_cmd)
  script = "(cd " + name + "; pycosim.profile inference_cosim.py --batch-size 4 > out.std 2>&1)"
