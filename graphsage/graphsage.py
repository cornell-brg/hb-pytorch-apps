...
class TorchNeighborhoodSampler:
    def __init__(self, adj, batch_size, n_neibs, hammerblade=True):
        ...
        np_offsets = np.random.randint(low=np.zeros(degrees.shape), high=degrees, size=(degrees.shape[0], n_neibs))
        np_offsets += indptr[:-1]
        np_neibs = indices[np_offsets]

        self.neibs = torch.IntTensor(np_neibs.astype(np.int32))

        self.n_neibs = n_neibs

        self.hammerblade = hammerblade
        if self.hammerblade:
            self.neibs = self.neibs.hammerblade()

    def sample_neighbors(self, nodes):

        return self.neibs[nodes].long()
...
class GraphSAGEModel(nn.Module):
    ...
    def forward(self, idx0):
        ...
        avg_2hop = avg_2hop_intermediate.sum(dim=-2) * 0.083333333
        ...
        avg1_hop = hidden_state1.sum(dim=-2) * 0.08333333
...
#print("Time for data loading and model initialization:", (end - start))
