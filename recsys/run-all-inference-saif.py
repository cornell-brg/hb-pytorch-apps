...
qsub_template = '(cd {path}; qsub -N {job_name} -l walltime=72:00:00 -l nodes=1:ppn=6 -l mem="20gb" -d {path} -o {path}/qsub.out -e {path}/qsub.err -V run.sh)\n'
...
for i in range(len(route)):
  ...
  name = "Recsys_inference_kernel_%03d_hb-saifgen" % i
  sh_cmd = "mkdir -p " + name
  print(sh_cmd)
  os.system(sh_cmd)
  ...
  # create run script
  script = "(source /work/global/lc873/work/sdh/venv_cosim/bin/activate; source /work/global/lc873/work/sdh/playground/bigblade-03-31/bsg_bladerunner/setup.sh; pycosim.saif /work/global/lc873/work/sdh/phase2-eval/recsys/hb-pytorch-apps/recsys/recsys.py --nbatch 1 --inference > out.std 2>&1; gzip run.saif;)\n"
